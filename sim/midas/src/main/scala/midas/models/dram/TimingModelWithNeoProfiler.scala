...
abstract class TimingModelWithNeoProfilerIO(implicit p: Parameters) extends Bundle {
...
abstract class TimingModelWithNeoProfiler(val cfg: BaseConfig)(implicit val p: Parameters) extends Module
  ...
  // if(true){
  val io: TimingModelWithNeoProfilerIO
  val longName: String
...
abstract class SplitTransactionModelWithNeoProfilerIO(implicit p: Parameters)
    extends TimingModelWithNeoProfilerIO()(p) {
  // This sub-bundle contains all the programmable fields of the model
  val mmReg: SplitTransactionMMRegIO
}

abstract class SplitTransactionModelWithNeoProfiler(cfg: BaseConfig)(implicit p: Parameters)
    extends TimingModelWithNeoProfiler(cfg)(p) {
  override val io: SplitTransactionModelWithNeoProfilerIO
