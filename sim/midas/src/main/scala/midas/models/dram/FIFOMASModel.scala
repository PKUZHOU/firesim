...
case class FIFOMASConfig(
  ...
  def elaborateWithNeoProfiler()(implicit p: Parameters): FIFOMASModelWithNeoProfiler = Module(new FIFOMASModelWithNeoProfiler(this)(p))
}
...
class FIFOMASWithNeoProfilerIO(val cfg: FIFOMASConfig)(implicit p: Parameters) extends TimingModelWithNeoProfilerIO()(p) {
  val mmReg = new FIFOMASMMRegIO(cfg)
  //override def clonetype = new FIFOMASIO(cfg)(p).asInstanceOf[this.type]
}
...
class FIFOMASModelWithNeoProfiler(cfg: FIFOMASConfig)(implicit p: Parameters) extends TimingModelWithNeoProfiler(cfg)(p)
    with HasDRAMMASConstants {
  ...
  lazy val io = IO(new FIFOMASWithNeoProfilerIO(cfg))
  val timings = io.mmReg.dramTimings
