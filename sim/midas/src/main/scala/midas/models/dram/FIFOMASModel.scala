...
case class FIFOMASConfig(
  ...
  def elaboratewithcmsketch()(implicit p: Parameters): FIFOMASModelWithCMsketch = Module(new FIFOMASModelWithCMsketch(this)(p))
}
...
class FIFOMASWithCMsketchIO(val cfg: FIFOMASConfig)(implicit p: Parameters) extends TimingModelWithCMsketchIO()(p) {
  val mmReg = new FIFOMASMMRegIO(cfg)
  //override def clonetype = new FIFOMASIO(cfg)(p).asInstanceOf[this.type]
}
...
class FIFOMASModelWithCMsketch(cfg: FIFOMASConfig)(implicit p: Parameters) extends TimingModelWithCMsketch(cfg)(p)
    with HasDRAMMASConstants {
  ...
  lazy val io = IO(new FIFOMASWithCMsketchIO(cfg))
  val timings = io.mmReg.dramTimings
