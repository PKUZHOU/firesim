...
case class BankConflictConfig(
  ...
  def elaborateWithNeoProfiler()(implicit p: Parameters):BankConflictModelWithNeoProfiler = Module(new BankConflictModelWithNeoProfiler(this))
}
...
class BankConflictWithNeoProfilerIO(cfg: BankConflictConfig)(implicit p: Parameters)
    extends SplitTransactionModelWithNeoProfilerIO()(p) {
  val mmReg = new BankConflictMMRegIO(cfg)
}
...
class BankConflictModelWithNeoProfiler(cfg: BankConflictConfig)(implicit p: Parameters) extends SplitTransactionModelWithNeoProfiler(cfg)(p) {

  val longName = "Bank Conflict"
  def printTimingModelGenerationConfig: Unit = {}
  /**************************** CHISEL BEGINS *********************************/
  // This is the absolute number of banks the model can account for
  lazy val io = IO(new BankConflictWithNeoProfilerIO(cfg))
