...
case class BankConflictConfig(
  ...
  def elaboratewithcmsketch()(implicit p: Parameters):BankConflictModelWithCMsketch = Module(new BankConflictModelWithCMsketch(this))
}
...
class BankConflictWithCMsketchIO(cfg: BankConflictConfig)(implicit p: Parameters)
    extends SplitTransactionModelWithCMsketchIO()(p) {
  val mmReg = new BankConflictMMRegIO(cfg)
}
...
class BankConflictModelWithCMsketch(cfg: BankConflictConfig)(implicit p: Parameters) extends SplitTransactionModelWithCMsketch(cfg)(p) {

  val longName = "Bank Conflict"
  def printTimingModelGenerationConfig: Unit = {}
  /**************************** CHISEL BEGINS *********************************/
  // This is the absolute number of banks the model can account for
  lazy val io = IO(new BankConflictWithCMsketchIO(cfg))
