...
case class FirstReadyFCFSConfig(
  ...
  def elaborateWithNeoProfiler()(implicit p: Parameters): FirstReadyFCFSModelWithNeoProfiler = Module(new FirstReadyFCFSModelWithNeoProfiler(this))
}
...
class FirstReadyFCFSWithNeoProfilerIO(val cfg: FirstReadyFCFSConfig)(implicit p: Parameters) extends TimingModelWithNeoProfilerIO()(p){
  val mmReg = new FirstReadyFCFSMMRegIO(cfg)
}
...
class FirstReadyFCFSModelWithNeoProfiler(cfg: FirstReadyFCFSConfig)(implicit p: Parameters) extends TimingModelWithNeoProfiler(cfg)(p)
    with HasDRAMMASConstants {
  ...
  import DRAMMasEnums._
  lazy val io = IO(new FirstReadyFCFSWithNeoProfilerIO(cfg))
