...
case class FirstReadyFCFSConfig(
  ...
  def elaboratewithcmsketch()(implicit p: Parameters): FirstReadyFCFSModelWithCMsketch = Module(new FirstReadyFCFSModelWithCMsketch(this))
}
...
class FirstReadyFCFSWithCMsketchIO(val cfg: FirstReadyFCFSConfig)(implicit p: Parameters) extends TimingModelWithCMsketchIO()(p){
  val mmReg = new FirstReadyFCFSMMRegIO(cfg)
}
...
class FirstReadyFCFSModelWithCMsketch(cfg: FirstReadyFCFSConfig)(implicit p: Parameters) extends TimingModelWithCMsketch(cfg)(p)
    with HasDRAMMASConstants {
  ...
  import DRAMMasEnums._
  lazy val io = IO(new FirstReadyFCFSWithCMsketchIO(cfg))
