...
class AXI4ReleaserWithCMsketchIO(implicit val p: Parameters) extends ParameterizedBundle()(p) {
  val b = Decoupled(new NastiWriteResponseChannel)
  val r = Decoupled(new NastiReadDataChannel)
  val egressReq = new EgressReq
  val egressResp = Flipped(new EgressResp)
...
class AXI4ReleaserWithCMsketch(implicit p: Parameters) extends Module {
  val io = IO(new AXI4ReleaserWithCMsketchIO)
