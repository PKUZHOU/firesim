...
case class LatencyPipeConfig(params: BaseParams) extends BaseConfig {
  ...
  def elaboratewithcmsketch()(implicit p: Parameters): LatencyPipeWithCMsketch = Module(new LatencyPipeWithCMsketch(this))
}
...
class LatencyPipeWithCMsketchIO(val cfg: LatencyPipeConfig)(implicit p: Parameters) extends SplitTransactionModelWithCMsketchIO()(p) {
  val mmReg = new LatencyPipeMMRegIO(cfg)
}
...
class LatencyPipeWithCMsketch(cfg: LatencyPipeConfig)(implicit p: Parameters) extends SplitTransactionModelWithCMsketch(cfg)(p)  {
  lazy val io = IO(new LatencyPipeWithCMsketchIO(cfg))
